...
@pytest.mark.asyncio
async def test_moderate_review_approve_success(review, client, mock_authenticated_admin):
    # Подготовка данных
    data = {
...
@pytest.mark.asyncio
async def test_moderate_review_unauthorized(review, client, mock_authenticated_superuser):
    data = {
        "review_id": str(review.id),
        "action": "approve"
    }
