...
from src.schemas.reviews import ModerateReviewSchema, ReviewOutSchema, PendingReviewSchema
from src.schemas.users import UserOutSchema, UserOutAdminSchema
...
from src.database.models import Review, House, User, Role
...
async def get_admin_stats(current_user: UserOutSchema = Depends(get_current_user)):
    ...
    try:
        ...
        # 6. Средний рейтинг
        published_reviews = await Review.filter(is_deleted=False, is_published=True).all()
        avg_rating = 0
        total = 0
...
async def get_pending_reviews(current_user: dict = Depends(get_current_user)):
    ...
    reviews: QuerySet[Review] = (
        Review.filter(is_published=False, is_deleted=False)
...
async def get_users(
    ...
    role: str | None = Query(None, description="Filter by role (Admin, Super User, User)"),
